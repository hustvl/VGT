...
def create_model_and_loss_module(config, logger, accelerator,
    ...
    if config.experiment.get("init_weight", ""):
        ...
        model_weight = torch.load(Path(config.experiment.init_weight) / "unwrapped_model/pytorch_model.bin", map_location="cpu")
...
def create_optimizer(config, logger, model, loss_module,
    ...
    encoder_lr_muti = getattr(config.optimizer, "encoder_lr_muti", 0.1)
    logger.info(f"Creating encoder_lr_muti: {encoder_lr_muti}")
    ...
    exclude_encoder = (lambda n, p: n.startswith('encoder.') and (p.ndim < 2 or "ln" in n or "bias" in n or 'latent_tokens' in n 
               or 'mask_token' in n or 'embedding' in n or 'norm' in n or 'gamma' in n or 'embed' in n))
    include_encoder = lambda n, p: not exclude_encoder(n, p) and n.startswith('encoder.')
    exclude_other = (lambda n, p: not n.startswith('encoder.') and (p.ndim < 2 or "ln" in n or "bias" in n or 'latent_tokens' in n 
               or 'mask_token' in n or 'embedding' in n or 'norm' in n or 'gamma' in n or 'embed' in n))
    include_other = lambda n, p: not exclude_other(n, p) and not n.startswith('encoder.')
    ...
    optimizer = optimizer_cls(
        [
            {"params": gain_or_bias_params_encoder, "weight_decay": 0., 'lr':learning_rate*encoder_lr_muti},
            {"params": rest_params_encoder, "weight_decay": optimizer_config.weight_decay, 'lr':learning_rate*encoder_lr_muti},
...
def create_dataloader(config, logger, accelerator):
    ...
    dataset = SimpleImageDataset(
        ...
        # max_samples_list=dataset_config.get('max_samples_list', [])
    )
...
def train_one_epoch(config, logger, accelerator,
    ...
    for i, batch in enumerate(train_dataloader):
        ...
        with accelerator.accumulate([model, loss_module]):
            ...
            if "min_encoding_indices" in extra_results_dict:
                indices = accelerator.gather(extra_results_dict["min_encoding_indices"].flatten().detach())
                codebook_size = config.model.bottleneck_config.codebook_size
                count_per_class = indices.bincount(minlength=codebook_size)
                vocab_usage = (count_per_class > 0).float().mean()
                autoencoder_logs["train/codebook_usage"] = vocab_usage.item()
            ...
            if eval_dataloader is not None and (global_step + 1) % config.experiment.eval_every == 0:
                ...
                if config.training.get("use_ema", False):
                    ...
                    if accelerator.is_main_process:
                        ...
                        wandb.log(eval_log, step=global_step + 1)
...
def eval_reconstruction(
    ...
    for batch in eval_progress_bar:
        ...
        if len(all_original_images) == eval_buffer_size:
...
def _process_buffer(all_original_images, all_reconstructed_images, accelerator, evaluator, eval_batch_size):
    all_original_images = torch.cat(all_original_images, dim=0).to(accelerator.device)
    all_reconstructed_images = torch.cat(all_reconstructed_images, dim=0).to(accelerator.device)
    ...
    # print("total_bs: ", total_bs)
    for i in range(0, total_bs, eval_batch_size):
        batch_original_images = all_original_images[i*eval_batch_size:(i + 1)*eval_batch_size, ...].to(accelerator.device)
        batch_reconstructed_images = all_reconstructed_images[i*eval_batch_size:(i + 1)*eval_batch_size, ...].to(accelerator.device)
        evaluator.update(batch_original_images, batch_reconstructed_images.squeeze(2), None)
...
def reconstruct_images(model, original_images, fnames, accelerator, 
    ...
    print(f"reconstructed_images: {reconstructed_images.min()}, {reconstructed_images.max()}")
