...
from .distill_loss import DistillLoss, SigLIP2_DistillLoss, Dinov3_DistillLoss, Dinohuf_DistillLoss, InternViT_DistillLoss, VLVAE_QWEN2_5_DistillLoss
...
class ReconstructionLoss_Single_Stage(ReconstructionLoss_Stage2):
    ...
    def _forward_generator(self,
        ...
        if self.quantize_mode in ["vq", "simvq", "ibq"]:
            # Compute quantizer loss.
            quantizer_loss = extra_result_dict["quantizer_loss"]
            total_loss = (
                reconstruction_loss
...
class VGTAE_ReconstructionLoss(torch.nn.Module):
    def __init__(
        self,
        config
    ):
    ...
    def _forward_generator(self,
        ...
        if "distill_loss" in extra_result_dict:
            distill_loss = extra_result_dict["distill_loss"]
        else:
            distill_loss = torch.zeros((), device=inputs.device)
        ...
        loss_dict = dict(
            total_loss=total_loss.clone().detach(),
            distill_loss=distill_loss.clone().detach(),
            reconstruction_loss=reconstruction_loss.detach(),
            perceptual_loss=(self.perceptual_weight * perceptual_loss).detach(),
            kl_loss=(self.kl_weight * kl_loss).detach(),
            weighted_gan_loss=(d_weight * discriminator_factor * generator_loss).detach(),
...
class VLVAE_QWEN2_5_Stage2_Loss(torch.nn.Module):
    def __init__(
        self,
        config
    ):
        ...
        self.perceptual_loss = PerceptualLoss(
            loss_config.perceptual_loss).eval()
        # self.distill_loss = VLVAE_QWEN2_5_DistillLoss(loss_config.distill_loss).eval()
        self.distill_weight = loss_config.distill_weight
    ...
    def _forward_generator(self,
        ...
        # else:
        #     raise ValueError(f"Unsuppored reconstruction_loss {self.reconstruction_loss}")
        reconstruction_loss = extra_result_dict["flow_loss"]
        ...
        total_loss = (
            ...
            # + d_weight * discriminator_factor * generator_loss
            + self.distill_weight * distill_loss
            # + self.align_dinov3_loss_weight * dinv3_align_loss
            # + self.align_siglip2_loss_weight * siglip2_align_loss
        )
        loss_dict = dict(
            total_loss=total_loss.clone().detach(),
            reconstruction_loss=reconstruction_loss.detach(),
