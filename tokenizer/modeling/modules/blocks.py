...
class TiTokDecoder(nn.Module):
    ...
    def forward(self, z_quantized, return_hidden_states=False):
        ...
        for i in range(self.num_layers):
            ...
            if return_hidden_states:
                return_lists.append(x.permute(1, 0, 2)[:, 1:1+self.grid_size**2])
        x = x.permute(1, 0, 2)  # LND -> NLD
        ...
        x = self.conv_out(x)
        if return_hidden_states:
            return x, return_lists
        return x

class TiTokEncoder2D(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.image_size = config.dataset.preprocessing.crop_size 
    ...
    def forward(self, pixel_values):
        ...
        # else:
        #     # Fix legacy problem.
        latent_tokens = latent_tokens.reshape(batch_size, self.grid_size, self.grid_size, self.width).permute(0, 3, 1, 2)
        latent_tokens = self.conv_out(latent_tokens)
        latent_tokens = latent_tokens.reshape(batch_size, self.token_size, self.grid_size, self.grid_size)
        return latent_tokens

class TiTokDecoder2D(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.image_size = config.dataset.preprocessing.crop_size
    ...
    def forward(self, z_quantized, return_hidden_states=False):
        N, C, H, W = z_quantized.shape
        assert H == self.grid_size and W == self.grid_size, f"{H}, {W}, {self.grid_size}"
        x = z_quantized.reshape(N, C, H*W).permute(0, 2, 1) # NLD
        x = self.decoder_embed(x)
        ...
        # mask_tokens = self.mask_token.repeat(batchsize, self.grid_size**2, 1).to(x.dtype)
        x = torch.cat([_expand_token(self.class_embedding, x.shape[0]).to(x.dtype),
                                    x], dim=1)
...
import math
class LightTiTok2DDecoder(TiTokDecoder):
    """
    轻量版 TiTokDecoder
    - 不生成像素
    - 输出 2D feature map
    ...
    def __init__(self, config, num_patch=256, feature_dim=768):
        super().__init__(config)

        self.config = config
        self.grid_size = int(math.sqrt(num_patch))
        self.patch_size = config.model.vq_model.vit_dec_patch_size
        ...
        # 替换掉像素的 FFN 和 conv_out，改为 feature 提取
        self.feature_head = nn.Sequential(
            nn.Conv2d(self.width, self.width, 1, bias=True),
            nn.GELU(),
            nn.Conv2d(self.width, feature_dim, 1, bias=True),
        )
    ...
    def forward(self, z_quantized, return_token_features=False):
        """
        输入：
            z_quantized: (N, C, 1, T) — latent tokens
        输出：
            dino-aligned 特征图: (N, H*W, feature_dim)
        """

        N, C, H, W = z_quantized.shape
        assert W == self.num_latent_tokens

        # ---- (1) 复用原始 TiTokDecoder 的 token embed ----
        x = z_quantized.reshape(N, C * H, W).permute(0, 2, 1)  # NLD
        x = self.decoder_embed(x)

        batchsize, seq_len, _ = x.shape

        # ---- (2) 原始的 mask token + cls + pos embed ----
        mask_tokens = self.mask_token.repeat(batchsize, self.grid_size**2, 1).to(x.dtype)
        mask_tokens = torch.cat([
            _expand_token(self.class_embedding, batchsize).to(x.dtype),
            mask_tokens
        ], dim=1)
        mask_tokens = mask_tokens + self.positional_embedding.to(x.dtype)
        ...
        # 去掉 cls token
        x = x[:, 1:1+self.grid_size**2]  # (N, L, width)

        x = self.ln_post(x)

        # ---- (4) reshape 到 2D grid ----
        H, W = self.grid_size, self.grid_size
        feat_2d = x.permute(0, 2, 1).reshape(batchsize, self.width, H, W)

        # ---- (5) 轻量 conv 提升特征表征 ----
        dino_aligned_feat = self.feature_head(feat_2d)

        return dino_aligned_feat
