...
class CaptionDataset(Dataset):
    def __init__(self,
        ...
        if train_number_samples is not None and train_number_samples > 0:
            if train_number_samples < len(self.data_list):
                self.data_list = self.data_list[:train_number_samples]
                print(f"Limited to {train_number_samples} samples (from {len(self.data_list)} total)", flush=True)
