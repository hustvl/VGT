...
def parse_args():
    ...
    parser.add_argument(
        "--csv",
        type=str,
        default='./scripts/evaluation/dpg_bench/dpg_bench.csv',
    )
    parser.add_argument(
        "--res-path",
        type=str,
...
class MPLUG(torch.nn.Module):
    def __init__(self, ckpt='/mmu-vcg-hdd/zhangshenhui/mplug_visual-question-answering_coco_large_en', device_index=0):
        ...
        self.pipeline_vqa = pipeline(Tasks.visual_question_answering, model=ckpt, device=device_str)
...
def main():
    ...
    local_rank = accelerator.local_process_index  # 0..7
    torch.cuda.set_device(local_rank)
    # vqa_model = MPLUG(device_index=local_rank)
    ...
    for fn in tqdm(local_filename_list):
        ...
        try:
            ...
            score, qid2tuple, qid2scores = compute_dpg_one_sample(
                args=args, question_dict=question_dict,
                image_path=image_path, vqa_model=vqa_model, resolution=args.resolution
            )
